...
These contracts are complete, auditable source — but there is no build
...
Once all four chain-related variables are set, `chain-adapter.ts` switches
...
- **Non-transferable by default.** Neither contract exposes `transfer`/
  `approve` — these are identity credentials, not tradable assets.
...
- **Replay-protected authorizations.** Every mint/upgrade requires an
  EIP-191 signature from `authorizedSigner` over a domain-separated message
  hash (`"FounderPassMint"` / `"BuilderPassMint"` / `"BuilderTierUpgrade"`
  prefixes), and each exact signed message can only be consumed once
  on-chain (`usedSignatures`). The backend additionally tracks its own
