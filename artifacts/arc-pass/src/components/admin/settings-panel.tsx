...
const INTEGRATIONS = [
  ...
  { name: "Explorer / indexer", vars: "EXPLORER_API_URL (optional), EXPLORER_API_KEY (custom providers only), ENVIO_HYPERSYNC_API_TOKEN (secondary fallback)", fallback: "Builder activity uses Arcscan by default. When Arcscan cannot finish pagination, Envio HyperSync is used if its server-only token is configured; safely captured counts are retained if both providers stop." },
